...
def process_seq(
    ...
    if not yolo_checkpoint and sam_version != "sam3_prompt_light":
        raise ValueError(
            "process_seq requires yolo_checkpoint (except for sam3_prompt_light, "
            "which detects via SAM3 text). Callers must supply a path to the "
            "YOLOv12-X weights — see --yolo-checkpoint on run_ma_masks.py. The "
            "MAMMA inference runner injects this from MAMMA_YOLO_CHECKPOINT / .env.local."
        )
